...
const Profile = ({user}) => {
  ...
  useEffect(()=>{
    setFormData({name:user.name,email:user.email,password:user.password,photo:user.photo,gender:user.gender,bloodgroup:user.bloodType})
},[user])

console.log('Formdatafromprofile',setFormData);
...
const navigate = useNavigate()
  ...
  return<section className='px-5 xl:px-0 '>
    <div className='max-w-[1170px] mx-auto flex-auto' >
      <div className='grid grid-cols-1 lg:grid-cols-2'>
            ...
            <div className='mb-5'>
            <input type='text' placeholder='Enter your blood group' name='bloodType' value={formData.bloodType} onChange={handleInputChange}
            className='w-full pr-4 py-3 border-b border-solid border-[#0066ff61] focus:outline-none focus:border-b-primaryColor text-[22px] leading-7 text-headingColor placeholder:text-textColor rounded-md cursor-pointer'
            required
            />
            </div>
            ...
            <div className='mt-7'>
            <button disabled={loading && true} type='submit' className='w-full bg-irisBlueColor text-white text-[18px] leading-[30px] rounded-lg px-4 py-3'>
