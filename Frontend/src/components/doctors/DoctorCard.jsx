...
const DoctorCard = ({ doctor, booking }) => {
  const data = booking || doctor;
  const {
    name,
    avgRating,
    totalRating,
    ...
    avgRating: 0, 
    totalRating: 0, 
    photo: booking.doctorPhoto,
    specialization: booking.specialization,
    experiences: [] 
  };

  const id = doctor ? data._id : booking.doctor; 
    ...
    <div className='p-3 lg:p-5'>
      ...
      {booking && booking.timeSlots && booking.timeSlots.length > 0 && (
        <div className='mt-2 lg:mt-4'>
          <h3 className='text-[16px] leading-7 lg:text-[18px] lg:leading-[30px] font-semibold text-headingColor'>Time Slot</h3>
          <p className='text-[14px] leading-6 font-[400] text-textColor'>
            {booking.timeSlots[0].day}, {booking.timeSlots[0].startingTime} - {booking.timeSlots[0].endingTime}
          </p>
        </div>
      )}
      {!booking &&
        ...
        <div>
          <h3 className='text-[16px] leading-7 lg:text-[18px] lg:leading-[30px] font-semibold text-headingColor'>+patients</h3>
          <p className='text-[14px] leading-6 font-[400] text-textColor'>At {experiences && experiences[0]?.hospital}</p>
        </div>
        <Link to={`/doctorsdetails/${id}`} className='w-[44px] h-[44px] rounded-full border border-solid border-[#181A1E] mt-[30px] mx-auto flex items-center justify-center group hover:bg-primaryColor hover:border-none'>
